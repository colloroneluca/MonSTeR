...
class TEMOS(LightningModule):
    ...
    def __init__(
        ...
        vae: bool,
        ts_encoder: nn.Module = None,
        tm_encoder: nn.Module = None,
        ms_encoder: nn.Module = None,
        fact: Optional[float] = None,
        sample_mean: Optional[bool] = False,
        lmd: Dict = {"recons": 1.0, "latent": 1.0e-5, "kl": 1.0e-5},
        lr: float = 1e-4,
        use_joint_encoder: bool = False,
    ...
    def _find_encoder(self, inputs, modality):
        ...
        if modality == "ts":
            ...
            return self.tm_encoder
        elif modality == "ms":
            return self.ms_encoder

        m_nfeats = self.motion_encoder.nfeats
        t_nfeats = self.text_encoder.nfeats
        if hasattr(self, "scene_encoder") and self.use_scene:
            s_feats = self.scene_encoder.nfeats
        else:
            s_feats = None
        ...
        if nfeats == m_nfeats:
            ...
            return self.text_encoder
        elif s_feats is not None and nfeats == s_feats:
            return self.scene_encoder
        else:
    ...
    def encode(
        ...
        joint_mod: bool = False,
    ):
        sample_mean = self.sample_mean if sample_mean is None else sample_mean
        fact = self.fact if fact is None else fact

        # Encode the inputs
        # NOTE: modality is 'auto'. This means that the encoder will be seclected on the base of
        #     the dimentionality of the input (e.g) if x.shape[-1]==263 then it is motion

        encoder = self._find_encoder(inputs, modality)

        if modality not in ["ts", "ms", "tm"]:
            encoded, tok_embeds, add_info = encoder(inputs)  # NOTE: shape => [32,2,256]
        else:
            encoded, tok_embeds, add_info = encoder(**inputs)
        ...
        if return_distribution:
            return (
                latent_vectors,
                dists,
                encoded if self.use_joint_encoder else None,
                tok_embeds,
                add_info,
            )
    ...
    def forward(
        ...
        # NOTE: sample_mean and fact are None
        if joint_mod:
            modality = joint_mod
        elif modality is None:
            modality = "auto"
        latent_vectors, distributions, encoded, tok_embeds, add_info = self.encode(
            inputs,
        ...
        # NOTE: in both cases possible here (if inputs contains motion or text) the latent
        #     will be decoded into a motion that will be returned together with latents and distribution
        if mask is not None:
    ...
    def training_step(self, batch: Dict, batch_idx: int) -> Tensor:
        batch["scene_x_dict"]["epoch"] = self.current_epoch
        batch["scene_x_dict"]["train_dl_len"] = self.train_dl_len
        batch["scene_x_dict"]["total_steps"] = self.total_steps
        bs = len(batch["motion_x_dict"]["x"])
        losses = self.compute_scene_loss(batch, phase="train")
